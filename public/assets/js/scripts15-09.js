(function () {
	...
	scheduler.templates.timeline_scale_label = function (key, label, section) {
		...
		var roomStatus = {"key":"3","label":"Availability/Rate"};
		// alert(JSON.stringify(roomStatus.label+'##'));
		// alert(JSON.stringify(scheduler.serverList("roomType")));
	...
	scheduler.attachEvent("onClone", function(start, end, event) {
	    // var ev = scheduler.getEvent(id);
	    event.location = "<b>Holder:</b>"+ event.location+"<br><b>Room:</b>"+ event.noofslots;
	});
	...
	scheduler.templates.tooltip_text = function (start, end, event) {
		...
		html.push("G&M Rate: <b>" + event.gm_rate + "</b>");
		html.push("P&R Rate: <b>" + event.pr_rate + "</b>");
	...
	window.updateSections = function updateSections(value) {
		...
		// for (var i = 0; i < 10; i++) {
			// if (value == roomsArr[i].type) {
				currentRoomsArr.push(roomsArr[3]);
			// }
		// }
...
function init() {
	...
	(function () {
		...
		var descriptionHTML = "<div class='timeline_item_separator'></div>" +
			"<div class='timeline_item_cell room_status'>For</div>";
